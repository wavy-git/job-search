...
> **PASSES IF:** tailoring workflow (Phase 1 strategy → Phase 2 by section → validation gates → ship); 10-point bullet framework; promotional reserve tables (interview-only material promotable to bullets); promotion rule (3 conditions); tailor flavor by company stage; skim vs narrative zone discipline; hidden-employer handling; side-by-side diff format; save format / naming convention; modern ATS reality reference.
>
> **FAILS IF:** master resume content (→ `me/resume.md`); voice rules applied to the writing (→ `me/voice.md`); facts about Maz used in bullets (→ `me/background.md`); JD body (→ `outputs/jds/`); the tailored output itself (→ `resumes/<jd>.docx`); JD collection (→ `skills/sourcing/`); JD scoring (→ `skills/screening/`); outreach drafts for the application (→ `skills/outreach/`); interview prep (→ `skills/interviewing/`); cover letters / essays (→ `skills/composing/`); cross-cutting authoring rules (→ `discipline.md`).
